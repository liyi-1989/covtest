...
load("results/simu_diffa/res.RData")
load("results/simu_diffa/results.df.RData")
...
#-------------- power curve ------------------------
dfepsub=dfep %>% filter(p==100,r==0.6,se==2,a%in%c(0.5,0.7),n>600,sig %in% c(0.1,0.2,0.3)) 

ggplot(dfepsub,aes(x=n,y=EP,colour=as.factor(hat1bar2)))+
  geom_line()+
  geom_point()+ 
  facet_grid(sig~a)+ 
  ylab("power")+
...
dfepsub=dfep %>% filter(p==200,n==2000,se==2,a%in%c(0.5,0.7),n>600,sig %in% c(0.1,0.2,0.3)) 

ggplot(dfepsub,aes(x=r,y=EP,colour=as.factor(hat1bar2)))+
  geom_line()+
  geom_point()+ 
  facet_grid(sig~a)+ 
  ylab("power")+
...
# sij
# df=as.data.frame(df)
# dfep$sij=c(mean1[,"sij_hat"],mean1[,"sij_bar"])
# dfepsub=dfep %>% filter(p==100,n==1000,se==2,a==0.6,r==0.5,n>600,sig %in% c(0.1,0.2,0.3)) 
# 
# ggplot(aes(y = sij, x = sig, fill= hat1bar2), data = dfepsub) + geom_boxplot()+ 
#   xlab("signal strength") + ylab("Estimation")+ scale_fill_discrete(name = "estimator")+theme(legend.position = c(0.8, 0.2))


dfsij=NULL 
for(jobid in 1:nrow(df)){
  load(paste0("results/simu_diffa/job_",jobid,".RData"))
  if(0.4<df[jobid,"r"]&df[jobid,"r"]<1& df[jobid,"a"] %in% c(0.5,0.7)& df[jobid,"sig"] %in% c(0.1,0.2,0.3) &all(df[jobid,c("n","p","se")]==c(1000,100,2))){
    temp=data.frame(r=df[jobid,"r"],a=df[jobid,"a"],sig=df[jobid,"sig"],sij=c(R1[,"sij_hat"],R1[,"sij_bar"]),hat1bar2=rep(1:2,each=nrow(R1)))
    dfsij=rbind(dfsij,temp)
  }
}
dfsij$r=as.factor(dfsij$r); dfsij$hat1bar2=as.factor(dfsij$hat1bar2)
ggplot(aes(y = sij, x = r, fill= hat1bar2), data = dfsij) + geom_boxplot()+ 
  facet_grid(sig~a)+ 
  xlab("signal strength") + ylab("Estimation")+ scale_fill_discrete(name = "estimator")+theme(legend.position = c(0.8, 0.2))
